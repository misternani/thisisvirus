...
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addquiz' && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        ...
        //imagequiz part
      if (@$_GET['q'] == 'addimagequiz' && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        $name    = $_POST['name'];
        $name    = ucwords(strtolower($name));
        $total   = $_POST['total'];
        $correct = $_POST['right'];
...
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addqns' && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') {
        ...
        for ($i = 1; $i <= $n; $i++) {
            ...
            $q3   = mysqli_query($con, "INSERT INTO questions VALUES  (NULL,'$eid','$qid','$qns' , '$ch' , '$i','$img')") or die();
            $oaid = uniqid();
            $obid = uniqid();
            $ocid = uniqid();
            $odid = uniqid();
...
//image quiz insert code
if (isset($_SESSION['key'])) {
    if (@$_GET['q'] == 'addimgqns' && $_SESSION['key'] == '54585c506829293a2d4c3b68543b316e2e7a2d277858545a36362e5f39') 
    {
        $n   = @$_GET['n'];
        $eid = @$_GET['eid'];
        $ch  = @$_GET['ch'];
        ...
        $fileName = basename($_FILES["file"]["name"][$i-1]);
    ...
    if(in_array($fileType, $allowTypes))
    {
        ...
        if(move_uploaded_file($_FILES["file"]["tmp_name"][$i-1], $targetFilePath))
        {

             $q3   = mysqli_query($con, "INSERT INTO questions VALUES  (NULL,'$eid','$qid','$fileName', '$ch' , '$i','$img')") or die();
            $oaid = uniqid();
            $obid = uniqid();
            $ocid = uniqid();
            $odid = uniqid();
...
//image quiz instertion code ends
