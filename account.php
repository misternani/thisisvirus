...
if ($rp) {
    ...
    echo '<script>console.log(-(window.history.length-1));</script>';
    echo '<script>console.log(window.history.length);</script>';
}
echo'<script>console.log(window.history.length);</script>';
...
$ttt=$_SESSION['username'];
$r = mysqli_query($con, "SELECT score from `rank` WHERE  username = '$ttt'") or die('Error69');
    while ($rol = mysqli_fetch_row($r)) {$ptt=$rol[0];}
   ...
   if (empty($ptt) || $ptt == 0) {
    ...
    if($ntt==1 && $_SESSION['isdisplayed']<=1){
        if($qtt=="quiz" && $stt=="2"){
       
         if(!$rp){
    echo '<div id="dialog-box" style="position: fixed; top: 0; left: 0; width: 100%; height: 100%; background-color: rgba(0, 0, 0, 0.5); z-index: 9999;">
              <div style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); background-color: white; padding: 20px; border-radius: 10px; text-align: center;">
                  <span style="position: absolute; top: 10px; right: 10px; font-size: 24px; font-weight: bold; cursor: pointer;" onclick="closeDialog()">&times;</span>
                  <p style="font-size: 18px; font-weight: bold;">Important Instruction: Click on <span class="glyphicon glyphicon-lock" style="font-size:12px;color:green" aria-hidden="true"></span> <span style="color:green">Save answer</span> button to save the answer .</p>
                  <button onclick="closeDialog()" style="background-color: #f44336; color: white; padding: 10px; border: none; border-radius: 5px; font-size: 16px; cursor: pointer;">Close</button>
              </div>
          </div>
          <script>
...
<tr><td style="vertical-align:middle"><b>S.N.</b></td><td style="vertical-align:middle"><b>Name</b></td><td style="vertical-align:middle"><b>Action</b></td></tr>';
    $c = 1;
  ...
  <td style="vertical-align:middle"><b><a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=1&t=' . $total . '&start=start" class="btn" style="margin:0px;background:darkorange;color:white">&nbsp;<span class="title1"><b>Continue</b></span></a></b></td></tr>';
                ...
                echo '<tr style="color:darkgreen"><td style="vertical-align:middle">' . $c++ . '</td><td style="vertical-align:middle">' . $title . '&nbsp;<span title="This quiz is already solve by you" class="glyphicon glyphicon-ok" aria-hidden="true"></span></td>
  <td style="vertical-align:middle"><b><a href="account.php?q=result&eid=' . $eid . '" class="btn" style="margin:0px;background:darkred;color:white">&nbsp;<span class="title1"><b>View Result</b></span></a></b>

       <b><a href="retake.php?eid=' . $eid . '&total='.$total.'" class="btn" style="margin:0px;background:darkblue;color:white">&nbsp;<span class="title1"><b>Clear</b></span></a></b>
...
function end(){
    ...
    sleep(02);
     window.location="result.php?eid='.$_GET["eid"]. '";
...
var countdownTimer = setInterval(\'secondPassed()\', 1000);
            ...
            while ($row = mysqli_fetch_array($q)) {
                ...
                $qid = $row['qid'];
                $c='uploads/'.$qns;
                $r     = mysqli_query($con, "SELECT image FROM questions WHERE eid='$eid' AND sn='$sn' ");
                while ($rol = mysqli_fetch_row($r)) {$pat=$rol[0];}
                if($pat)
                {
                ...
                echo '<b><div style="font-size:20px;font-weight:bold;font-family:calibri;margin:10px;word-wrap:break-word">' . $sn . ' : ' . $qns . '</div></b>';
                }
            }
...
<br />';
            ...
            if (strlen($ans) > 0) {
                echo "<font style=\"color:green;font-size:12px;font-weight:bold\">Selected answer: </font><font style=\"color:#565252;font-size:12px;\">" . $ans . "</font>&nbsp;&nbsp;<a href=update.php?q=quiz&step=2&eid=$eid&n=$sn&t=$total&qid=$qid&delanswer=delanswer><span class=\"glyphicon glyphicon-remove\" style=\"font-size:12px;color:white\"></span></a><br /><br />";
            }
            ...
            while ($row = mysqli_fetch_array($q)) {
                ...
                echo '<div class="funkyradio-success"><input type="radio" id="' . $optionid . '" name="ans" value="' . $optionid . '" onclick="enable()"> <label for="' . $optionid . '" style="width:100%;text-align:center"><div style="color:black;font-size:12px;word-wrap:break-word">&nbsp;&nbsp;' . $option . '</div></label></div>';
            }
            echo '</div>';
            if ($_GET["t"] > $_GET["n"] && $_GET["n"] != 1) {
                echo '<br /><a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=' . ($sn - 1) . '&t=' . $total . '" class="btn btn-primary" style="height:30px"><span class="glyphicon glyphicon-arrow-left" aria-hidden="true"  style="font-size:12px"></span></a>&nbsp;&nbsp;&nbsp;&nbsp;<button type="submit" class="btn btn-default" disabled="true" id="sbutton" style="height:30px"><span class="glyphicon glyphicon-lock" style="font-size:12px;color:green" aria-hidden="true"></span><font style="font-size:12px;font-weight:bold;color:green"> save answer</font></button>&nbsp;&nbsp;&nbsp;&nbsp;<button type="button" class="btn btn-default" onclick="frmreset()" style="height:30px;display:none"></span><font style="font-size:12px;font-weight:bold">Reset</font></button>&nbsp;&nbsp;&nbsp;&nbsp;<a href="account.php?q=quiz&step=2&eid=' . $eid . '&n=' . ($sn + 1) . '&t=' . $total . '" class="btn btn-primary" style="height:30px"><span class="glyphicon glyphicon-arrow-right" aria-hidden="true"  style="font-size:12px"></span></a></form><br><br>';
...
<center><h1 class="title" style="color:#660033">Result</h1><center><br /><table class="table table-striped title1" style="font-size:20px;font-weight:1000;">';
    ...
    <tr style="color:darkblue"><td style="vertical-align:middle">Score&nbsp;<span class="glyphicon glyphicon-star" aria-hidden="true"></span></td><td style="vertical-align:middle">' . $s . '</td></tr>';
        ...
        die("You must submit  any opened tests before accessing new test so now go and submit continue or opened test and submit the quiz.");
    }
}
if (@$_GET['q'] == 2) {
